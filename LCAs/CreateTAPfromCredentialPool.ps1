...
if ($UserName -ne $null -or $UserName -ne '') {
    ...
    try {
        ...
        if ($ScriptDebug) { Write-Output "Successfully acquired access token" }
    } catch {
        if ($ScriptDebug) { Write-Output "Failed to acquire access token: $($_.Exception.Message)" }
        return $false
    }
    ...
    try {
        $createUserResponse = Invoke-RestMethod -Method GET `
            ...
            -Headers $headers `
            -ErrorAction Stop
        if ($ScriptDebug) { Write-Output "User $TapUser exists" }
    } catch {
        if ($_.Exception.Response.StatusCode -eq 404) {
            if ($ScriptDebug) { Write-Output "User $TapUser does not exist, creating..." }
            $userDetails = @{
                "accountEnabled"    = $true
            ...
            try {
                $createUserResponse = Invoke-RestMethod -Method POST `
                    ...
                    -ErrorAction Stop
                if ($ScriptDebug) { Write-Output "User $TapUser created successfully" }
            } catch {
                if ($ScriptDebug) { Write-Output "Failed to create user $TapUser : $($_.Exception.Message)" }
            }
        } else {
            if ($ScriptDebug) { Write-Output "Error checking user $TapUser : $($_.Exception.Message)" }
        }
    }
    ...
    try {
        ...
        if ($ScriptDebug) { Write-Output "Global Administrator role assigned to $TapUser" }
    } catch {
        if ($_.Exception.Response.StatusCode -eq 409) {
            if ($ScriptDebug) { Write-Output "Global Administrator role already exists for $TapUser" }
        } else {
            if ($ScriptDebug) { Write-Output "Failed to assign Global Administrator role to $TapUser : $($_.Exception.Message)" }
        }
    }
    ...
    try {
        ...
        if ($availableSkus.Count -eq 0 -and $ScriptDebug) { Write-Output "No available licenses found in tenant" }
    } catch {
        if ($ScriptDebug) { Write-Output "Failed to retrieve SKUs: $($_.Exception.Message)" }
    }
    ...
    if ($availableSkus.Count -gt 0) {
        ...
        try {
            Invoke-RestMethod -Method POST `
                ...
                -ErrorAction Stop | Out-Null
            if ($ScriptDebug) { Write-Output "Licenses assigned to $TapUser" }
        } catch {
            if ($_.Exception.Response.StatusCode -eq 400 -and $_.Exception.Message -match "already assigned") {
                if ($ScriptDebug) { Write-Output "Licenses already assigned to $TapUser" }
            } else {
                if ($ScriptDebug) { Write-Output "Failed to assign licenses to $TapUser : $($_.Exception.Message)" }
            }
        }
    }
    ...
    try {
        Invoke-RestMethod -Method PATCH `
            ...
            -ErrorAction Stop
        if ($ScriptDebug) { Write-Output "TAP policy updated to allow multiple uses" }
    } catch {
        if ($ScriptDebug) { Write-Output "Failed to update TAP policy: $($_.Exception.Message)" }
    }
    ...
    try {
        ...
        if ($ScriptDebug) { Write-Output "TAP Password: $TapPassword created for: $TapUser" }
    } catch {
        if ($ScriptDebug) { Write-Output "Failed to create TAP for $TapUser : $($_.Exception.Message)" }
    }
    ...
    if ($ScriptDebug) { Write-Output "Credential Pool not available. Falling back on manual credentials." }
}
