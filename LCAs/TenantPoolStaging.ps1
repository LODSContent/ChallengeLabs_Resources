...
if ($graphSp) { 
	if ($ScriptDebug) {Send-DebugMessage "Microsoft Graph Service Principal found in $TenantName"}
	...
	if ($Permissions) {
		...
		$indexedPermissions | ForEach-Object -ThrottleLimit 25 -Parallel {
			...
			if ($role) {
				...
				if ($Using:ScriptDebug) { Send-DebugMessage "App role $permission not found in Microsoft Graph" }
			}
		} 
	}               
	if ($ScriptDebug) {Send-DebugMessage "Finished updating permissions for: $TenantName"}
} else {
	if ($ScriptDebug) {Send-DebugMessage "Microsoft Graph Service Principal not found in $TenantName"}
}
