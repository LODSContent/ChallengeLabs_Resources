...
If ($ipmsg1 -ne $null){    
    ...
    if ($response -eq 'y'){
        ...
        New-NetIPAddress -InterfaceIndex $ifindex -IPAddress 172.16.0.21 -PrefixLength $pf -DefaultGateway 172.16.0.1
        $NewDNS = Set-DnsClientServerAddress -InterfaceIndex $ifindex -ServerAddresses 172.16.0.254
        $ipconfig = Get-NetIPConfiguration
        $ip = $ipconfig.Ipv4Address.IPaddress
        $pf = $ipconfig.IPv4Address.PrefixLength
        $dg = $ipconfig.IPv4DefaultGateway.NextHop
