# Kubuntu 26.04 — TurboVNC with Login Screen Setup

This document covers the complete installation and configuration of TurboVNC with a GTK login screen on Kubuntu 26.04, suitable for Skillable lab environments where students may need to log in as different user accounts.
    ...
    while True:
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if result.returncode == 0:
                ...
                subprocess.run(["su", "-", username, "-c", "DISPLAY=:1 startplasma-x11"], env=env)
                return
            else:
...
## Step 9: Configure default KDE Plasma environment for new users

This copies the KDE Plasma desktop configuration from the initial user account into `/etc/skel` so that any new user created on the system automatically gets a properly configured desktop.

Replace `labuser` below with the name of your configured base user if different:

```bash
mkdir -p /etc/skel/.config
cp -r /home/labuser/.config/plasma* /etc/skel/.config/
cp -r /home/labuser/.config/kde* /etc/skel/.config/
cp -r /home/labuser/.config/kwin* /etc/skel/.config/
cp -r /home/labuser/.local/share/plasma* /etc/skel/.local/share/ 2>/dev/null || true
```
...
Any new user created with the `-m` flag will automatically inherit the KDE Plasma configuration from skel:
...
**Plasma session fails to start / black screen after login:**
KDE Plasma requires a D-Bus session. If the session launches to a black screen or crashes immediately, test manually as labuser first:

```bash
su - labuser -c "DISPLAY=:1 dbus-launch startplasma-x11"
```

If that succeeds, update the login script's `subprocess.run` call in `/usr/local/bin/vnc-login` to wrap the command:

```python
subprocess.run(["su", "-", username, "-c", "DISPLAY=:1 dbus-launch startplasma-x11"], env=env)
```
...
- After a user logs out of KDE Plasma, the login dialog automatically reappears for the next user.
...
- Openbox runs as a minimal window manager during the login screen only to enforce click-to-focus behavior. It is replaced by the KDE Plasma window manager (KWin) once a user logs in.
- The physical Hyper-V desktop remains available on display `:0` via SDDM and is unaffected by the VNC session on `:1`.
- Kubuntu 26.04 retains X11 session support for KDE Plasma (`startplasma-x11`). If this command is not found, verify the `plasma-workspace` package is installed: `apt install plasma-workspace`.
