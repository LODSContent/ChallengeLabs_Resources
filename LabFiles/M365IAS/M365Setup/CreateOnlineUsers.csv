...
#Create users and ensure they are assigned appropriate licences

Import-Csv D:\LabFiles\M365Setup\onlineusers.csv | foreach {New-MsolUser -UserPrincipalName ($_.SAM + "$dn")  -DisplayName "$($_.DisplayName)" -FirstName "$($_.Fname)" -LastName "$($_.Lname)" -Department "$($_.Department)" -City "$($_.City)" -State "$($_.State)" -Title "$($_.Title)"-Password 'Passw0rd!' -PasswordNeverExpires $true -ForceChangePassword $false -UsageLocation "US"}  
...
Foreach ($ID in $NewUserIDs) {
 ...
 # by pausing for a period of time and then rerumming portions of the script.
...
If ($nMa -lt 4)
 ...
 Else
  ...
  write-host "" 
  Write-host "Please proceed with the lab." -ForegroundColor Green
  }
